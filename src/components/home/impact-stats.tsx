...
export function ImpactStats() {
  return (
    <section
      className="border-y border-sage-100/80 bg-gradient-to-b from-sage-50/50 to-cream py-16 sm:py-20"
        ...
        <dl className="mt-12 grid grid-cols-2 gap-6 lg:grid-cols-4 lg:gap-8">
          {impactStats.map((stat, index) => (
            <FadeIn key={stat.label} delay={index * 0.06}>
              <div className="flex flex-col items-center rounded-2xl bg-white/80 px-4 py-8 text-center shadow-sm ring-1 ring-sage-100/60">
                <div
                  className="mb-3 flex h-11 w-11 items-center justify-center rounded-full bg-mist-100 text-mist-400"
                  aria-hidden
                >
                  <StatIcon name={stat.icon} className="h-5 w-5" />
                </div>
                <dt className="sr-only">{stat.label}</dt>
                <dd className="font-display text-3xl font-bold text-sage-600 sm:text-4xl">
                  {stat.value}
                </dd>
